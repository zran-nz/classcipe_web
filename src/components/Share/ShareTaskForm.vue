<template>
  <div class='share-task-form'>
    ...
    <div class='form-item'>
      ...
      <div class='link-list'>
        <div class='link-item' v-for='(link, lIndex) in shareContent.task.materialList' :key='lIndex'>
          <div class='link-name'>{{ link.name }}</div>
          <div class='link-url' @click='openLink(link.link)'> {{ link.link }}</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    assessmentObjectives: function () {
      return this.shareContent.task.learnOuts.filter(item => item.tagType === 5)
    },
    learningOutcomes: function () {
      return this.shareContent.task.learnOuts.filter(item => item.tagType === 1)
    },
    centurySkills: function () {
      return this.shareContent.task.learnOuts.filter(item => item.tagType === 4)
    }
  },
  methods: {
    openLink (link) {
