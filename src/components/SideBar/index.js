...
export default {
  ...
  data () {
    return {
      openKeys: [],
      selectedKeys: [this.initSelected],
      cachedOpenKeys: [],
      menu: this.menus
    }
  },
  ...
  methods: {
    ...
    // render
    renderItem (menu) {
      if (!menu.hidden && !this.hiddenRoute(menu)) {
    ...
    renderMenuItem (menu) {
      ...
      return (
        ...
        {...{ key: menu.path,
          ...
          'selected-nav-bar': this.selectedKeys.includes(menu.path)
        } }}
        >
          <tag {...{ props, attrs }}>
    ...
    renderIcon (icon) {
      ...
      const props = { theme: this.iconTheme }
      typeof (icon) === 'object' ? props.component = icon : props.type = icon
      return (
        <Icon class="nav-bar-icon" {... { props } }/>
      )
    }
  },

  render () {
    const { mode, theme, menu, hiddenRoute } = this
    const props = {
      mode: mode,
      theme: theme,
    ...
    const menuTree = menu.map(item => {
      if (item.hidden || hiddenRoute(item)) {
        return null
      }
