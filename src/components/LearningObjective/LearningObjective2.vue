        ...
        <div class='filter-line'>
          <div class='select-item'>
            ...
            <a-select
              ...
              <a-select-option :value='item.code' v-for='(item, index) in curriculums' :key='index'>
                {{ item.name }}
              </a-select-option>
            </a-select>
...
export default {
  ...
  methods: {
    currChangeFn(val) {
      console.info(val, this.curriculum)
    },
    async initData() {
      ...
      const rs = await App.service('curriculum').get('pubList')
      console.log('getAllCurriculums', rs)
      this.curriculums = rs
      // this.init()
      console.log('LearningObjective init done', this.filterConfig)
      this.loading = false
      // this.startWatch()
    },
