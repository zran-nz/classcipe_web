<template>
  <div class="topic-preview">
    ...
    <template v-else>
        ...
        <a-col span="24">
          ...
          <template v-if="lastChangeSavedTime">
            ...
            <span class="last-change-time">
              {{ $t('teacher.add-topic.last-change-saved-at-time', {time: lastChangeSavedTime}) }}
            </span>
          </template>
        </a-col>
      </a-row>
          ...
          <div class="detail-wrapper">
            <div class="detail-block">
              <div class="block-title">
                {{ topicData.scenario && topicData.scenario.description }}
                <span class="title-icon">
                  <a-tooltip>
                    <template slot="title">
                      {{ $t('teacher.topic-preview.scenario-description') }}
                    </template>
              ...
              <div class="block-content">
                <div class="content-list" v-if="topicData.scenario && topicData.scenario.sdgKeyWords">
                  <div class="content-item" v-for="(sdgKeyword,index) in topicData.scenario.sdgKeyWords" :key="index">
                    <div class="question">
            ...
            <div class="detail-block">
              <div class="block-title">
                ...
                <span class="title-icon">
                  <a-tooltip>
                    <template slot="title">
                      {{ $t('teacher.topic-preview.direction-of-inquiry') }}
                    </template>
                    <a-icon type="info-circle" />
                  </a-tooltip>
                </span>
              ...
              <div class="block-content">
                <div class="content-list" v-if="topicData.questions && topicData.questions.length">
                  <div class="content-item" v-for="(question,qIndex) in topicData.questions" :key="qIndex">
                    <div class="question">
                      {{ question.name }}
                    </div>
      ...
      <topic-associate-preview :topic-id="topicId" v-if="showAssociate"/>
    </template>
  </div>
</template>
...
import TopicAssociatePreview from './TopicAssociatePreview'
...
export default {
  ...
  components: {
    TopicAssociatePreview
  },
  computed: {
    lastChangeSavedTime () {
      if (this.topicData) {
        logger.info('lastChangeSavedTime topicData', this.topicData)
        const time = this.topicData.createTime || this.topicData.updateTime
        if (time) {
  ...
  created () {
    logger.info('TopicPreview topicId ' + this.topicId)
    this.loadTopicData()
  },
  methods: {
    loadTopicData () {
      logger.info('loadTopicData ' + this.topicId)
      this.loading = true
      TopicQueryById({
        id: this.topicId
      }).then(response => {
        logger.info('TopicQueryById ' + this.topicId, response.result)
        this.topicData = response.result
        if (this.topicData && this.topicData.image) {
          this.imgList = [this.topicData.image]
        }
      }).finally(() => {
        this.loading = false
      })
