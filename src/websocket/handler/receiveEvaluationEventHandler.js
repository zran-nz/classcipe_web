...
export default class ReceiveEvaluationEventHandler extends AbstractMessageHandler {
  ...
  processMessage(proto) {
    // 通知更新系统消息
    logger.info('ReceiveEvaluationEventHandler : ', proto.cmd)
    switch (proto.cmd) {
      case EVALUATION_SET_CHANGE:
        EvaluationEventBus.$emit(EVALUATION_SET_CHANGE, proto.data)
        break
      case EVALUATION_ATTENDANCE_CHANGE:
        EvaluationEventBus.$emit(EVALUATION_ATTENDANCE_CHANGE, proto.data)
        break
      case EVALUATION_TEACHER_SUBMMIT:
        EvaluationEventBus.$emit(EVALUATION_TEACHER_SUBMMIT, proto.data)
        break
      case EVALUATION_STUDENT_SUBMMIT:
        EvaluationEventBus.$emit(EVALUATION_STUDENT_SUBMMIT, proto.data)
        break
      case EVALUATION_PEER_SUBMMIT:
        EvaluationEventBus.$emit(EVALUATION_PEER_SUBMMIT, proto.data)
        break
      default:
