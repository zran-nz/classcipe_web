    ...
    <a-card :bordered="false" :bodyStyle="{ padding: '16px 24px', height: '100%', minHeight: '1200px' }">
                  ...
                  <div class="collaborate-panel" :style="{'width':'600px', 'margin-top': '0px', 'z-index': 100, 'padding': '10px'}">
                    ...
                    <a-tabs default-active-key="1">
                      <a-tab-pane key="1" tab="Comment">
                        <collaborate-comment-view :source-id="topicId" :source-type="contentType.topic" :comment-list="collaborateCommentList" @update-comment="handleUpdateCommentList"/>
                      </a-tab-pane>
              ...
              <template v-else>
                <template v-if="showCollaborateCommentVisible">
                  <div class="collaborate-panel" :style="{'width':'600px', 'margin-top':collaborateTop+'px', 'z-index': 100, 'padding': '10px'}">
                    <collaborate-comment-panel :source-id="topicId" :source-type="contentType.topic" :field-name="currentFieldName" :comment-list="currentCollaborateCommentList" @update-comment="handleUpdateCommentList"/>
                  </div>
                </template>
                <template v-else>
      ...
      <a-modal
        ...
        <div class="add-content-wrapper">
          <div class="add-content-item" @click="handleAddTopicTask">
            <a>
              <content-type-icon :type="contentType.task"/>
              {{ $t('teacher.add-unit-plan.task') }}
            </a>
          </div>
          <div class="add-content-item" @click="handleAddTopicLesson">
            <a >
              <content-type-icon :type="contentType.lesson"/>
              {{ $t('teacher.add-unit-plan.lesson') }}
            </a>
          </div>
          <!--          <div class="add-content-item" @click="handleAddTopicEvaluation">-->
          <!--            <a >-->
      ...
      <a-modal
        ...
        <div class="link-content-wrapper">
          <my-content-selector :current-id="topicId" :filter-type-list="['.topic', 'topic']" :mode="DisplayMode.Refer" />
        </div>
      </a-modal>
...
export default {
  ...
  methods: {
    ...
    loadCollaborateData () {
      return Promise.all([
        GetCollaborateModifiedHistory({ sourceType: this.contentType.topic, sourceId: this.form.id }),
        GetCollaborateComment({ sourceType: this.contentType.topic, sourceId: this.form.id })
      ]).then(response => {
    ...
    restoreTopic (topicId, isFirstLoad) {
      ...
      TopicQueryById({
        ...
        if (topicData.scenarios.length === 0) {
          ...
          topicData.scenarios.forEach(item => {
            if (!item.sdgId) {
    ...
    handlePublishTopic () {
      ...
      this.form.status = 1
      TopicAddOrUpdate(this.form).then(() => {
    ...
    handleAddTopicMaterial () {
      ...
      this.$router.push({
        path: '/teacher/.topic-material-redirect/' + this.topicId + '/create'
      })
    },

    handleAddTopicTask () {
      this.$logger.info('handleAddTopicTask ' + this.topicId)
      ...
      if (!this.addLoading) {
        ...
        LessonAddOrUpdate({
          ...
          if (response.success) {
            ...
            this.addLoading = false
            this.$router.push({
              path: '/teacher/.topic-lesson-redirect/' + response.result.id
            })
          } else {
            this.$message.error(response.message)
          }
    ...
    handleAddTopicEvaluation () {
      logger.info('handleAddTopicEvaluation ' + this.topicId)
    ...
    handleCloseReferDetail () {
      ...
      this.referId = null
      this.referType = null
      this.$refs['form'].className = '.topic-form-left'
    },

    handleHoverReferBlock (data) {
      this.$logger.info('handleHoverReferBlock', data)
      this.$refs['form'].className = '.topic-form-left ' + data.blockType
    ...
    getAssociate () {
      this.$logger.info('AddTopic GetAssociate id[' + this.topicId + '] fromType[' + this.contentType.topic + ']')
      GetAssociate({
        id: this.topicId,
        type: this.contentType.topic
      }).then(response => {
        this.$logger.info('AddTopic GetAssociate response', response)
        this.groupNameList = []
