...
export default {
  ...
  watch: {
    currentActiveFormId (formId) {
      console.info('update currentActiveFormId ' + formId)
      // 判断当前的formId是否是允许他评
        ...
        this.allStudentUserIdList.forEach(studentId => {
          ...
          rowIdList.forEach(rowId => {
            ...
            if (rowData && rowData.peerEmail && this.currentUserGroupUserIdList.indexOf(rowData.peerEmail) !== -1) {
              console.info('student ' + studentId + ' row ' + rowId + ' has peerEvaluation')
  ...
  created () {
    console.info('[' + this.mode + '] created ClassSessionEvaluation sessionId' + this.sessionId)
  ...
  methods: {
    initClassSessionEvaluation () {
      console.info('initClassSessionEvaluation sessionId ' + this.sessionId)
    ...
    loadClassSessionEvaluationData () {
      ...
      GetSessionEvaluationByClassId({ classId: this.sessionId }).then(response => {
        ...
        data.groups.forEach(group => {
          ...
          group.members.forEach(member => {
            ...
            if (member.userId === this.$store.getters.userInfo.email) {
              ...
              console.info('currentUserGroupId' + this.currentUserGroupId, 'currentUserGroupUserIdList', this.currentUserGroupUserIdList)
            }
          })
        })
        ...
        if (!isEmptyStudentEvaluateData) {
          ...
          console.info('exist studentEvaluateData', studentEvaluateData)
          // 处理新增的学生
          allStudentUserIdList.forEach(studentId => {
            if (!studentEvaluateData.hasOwnProperty(studentId)) {
              console.info('new student', studentId)
          ...
          if (this.mode === EvaluationTableMode.Preview) {
            ...
            console.info('currentActiveFormId ' + this.currentActiveFormId + ' currentActiveStudentId ' + this.currentActiveStudentId)
          }
        }
        ...
        if (this.mode === EvaluationTableMode.PeerEvaluate) {
          if (this.currentActiveFormId) {
            this.allowPeerEvaluate = true
            this.allStudentUserIdList.forEach(studentId => {
              console.info('studentId ' + studentId + ' currentActiveFormId ', this.currentActiveFormId)
              const currentFormData = this.studentEvaluateData[studentId][this.currentActiveFormId]
              const rowIdList = Object.keys(currentFormData)
              rowIdList.forEach(rowId => {
                const rowData = currentFormData[rowId]
                console.info('currentFormData', currentFormData, 'rowId', rowData)
                if (rowData && rowData.peerEmail && this.currentUserGroupUserIdList.indexOf(rowData.peerEmail) !== -1) {
                  console.info('student ' + studentId + ' row ' + rowId + ' has peerEvaluation')
                  this.allowPeerEvaluate = false
                }
      ...
      if (this.mode === EvaluationTableMode.PeerEvaluate) {
        if (!this.allowPeerEvaluate) {
          ...
          this.$confirm({
            ...
            centered: true,
            onOk: () => {
              console.info('currentActiveStudentId', member)
    ...
    handleSelectGroup (group) {
      ...
      if (this.mode === EvaluationTableMode.PeerEvaluate) {
        if (this.mode === EvaluationTableMode.PeerEvaluate && group.id === this.currentUserGroupId) {
          ...
          console.info('handleSelectGroup selectedMemberIdList', this.selectedMemberIdList)
        }
      } else {
    ...
    handleSaveEvaluation (switchEvaluate) {
      console.info('handleSaveEvaluation', this.forms)
