...
export default {
  ...
  methods: {
    ...
    downloadTemplate() {
      ...
      const url = this.baseUrl + '/classcipe/excel/school_students_template.xls'
      link.href = url
    ...
    handleMyImportExcel(data) {
      ...
      this.$http
        .post(schoolUserAPIUrl.importStudent, formData, {
          contentType: false,
          processData: false,
          ...
          if (res.success) {
            ...
            if (invalidCount === 0) {
              ...
              this.$warning({
                ...
                onOk() {
                  const link = document.createElement('a')
                  const objectUrl = `${process.env.VUE_APP_API_BASE_URL}${schoolUserAPIUrl.exportInvalidStudent}?key=${invalidDataKey}`
                  link.href = objectUrl
                  link.download = 'invalidStudent.xls' // 自定义文件名
                  link.click() // 下载文件
