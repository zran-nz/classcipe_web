...
export default {
  ...
  async created() {
    ...
    console.info('add task created ' + this.taskId + ' ' + this.$route.path + ' mode: ' + this.mode, this.$route.params)
  ...
  methods: {
    ...
    updateUnitIdList (idList) {
      console.info('associateUnitPlanIdList', idList)
      ...
      const redirectPath = this.form.parentTaskId ? '/teacher/sub-task/' + this.form.parentTaskId : '/'
       if (this.canEdit) {
         if (this.currentActiveStepIndex === this.formSteps.length - 1) {
           ...
           console.info('notRemind ', notRemind)
    ...
    restoreTask(taskId, needLoadThumbnail = true) {
      ...
      TaskQueryById({
        ...
        console.info('restoreTask done', this.form)
    ...
    handlePublishTask() {
      console.info('handlePublishTask', {
        id: this.taskId,
        status: 1
      })
    ...
    async handleEditGoogleSlide() {
      console.info('handleEditGoogleSlide star')
      this.$store.commit(SET_GLOBAL_LOADING, true)
      this.$nextTick(async () => {
        try {
          this.editGoogleSlideLoading = true
          console.info('handleEditGoogleSlide', this.form.presentationId, this.$store.getters.globalLoading)
    ...
    getAssociate() {
      ...
      GetAssociate({
        ...
        console.info('associateQuestionList', this.associateQuestionList)
        this.requiredFields = this.$classcipe.taskRequiredFields
      }).finally(() => {
    ...
    handleUpdateSelfOuts (data) {
      console.info('handleUpdateSelfOuts', data)
      ...
      console.info('selfOuts', selfOuts)
    },
    ...
    handleDeleteClass (idx, classItem) {
      console.info('handleDeleteClass', classItem)
    ...
    changeSelected(checked) {
      console.info('changeSelected ', checked)
    ...
    handleGoToSubTask (data) {
      console.info('handleGoToSubTask', data)
      this.$router.replace({
