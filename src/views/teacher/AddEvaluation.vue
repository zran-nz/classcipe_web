        ...
        <a-card :bordered="false" :style="{ borderLeft: '1px solid rgb(235, 238, 240)'}" :body-style="{padding: '16px'}">
          <div class="rubric-wrapper">
            <div class="rubric-item" v-if="form.tableMode === 1 ">
              <rubric-one ref="rubric" :description-list="evaluationTableList"/>
            </div>
          </div>
        </a-card>
      </a-col>
...
export default {
  ...
  methods: {
    ...
    restoreEvaluation (evaluationId, isFirstLoad) {
      if (isFirstLoad) {
        this.contentLoading = true
      }
      logger.info('restoreEvaluation ' + evaluationId)
      EvaluationQueryById({
        id: evaluationId
      }).then(response => {
        logger.info('EvaluationQueryById ' + evaluationId, response.result)
        const evaluationData = response.result
        this.form = evaluationData
        logger.info('after restoreEvaluation', this.form)
      }).finally(() => {
        this.contentLoading = false
      })
    },
    ...
    handleSaveEvaluation () {
      logger.info('handleSaveEvaluation', this.form, this.$refs.rubric.headers, this.$refs.rubric.list)
