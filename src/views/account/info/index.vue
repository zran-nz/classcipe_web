        ...
        <div class="info-header-detail">
          ...
          <div class="header-detail-email" v-if="userMode === USER_MODE.SELF">
            <label>
              ...
              <a-tooltip v-if="info.emailVerify === 0">
                <div slot="title">
                  <label for="">
                    The inactive account will be automatically removed after 30 days upon registration. Click to resend
                    an email with activating link</label
                  >
                  <div style="text-align:right;">
                    <a-button type="primary" :loading="sendLoading" @click="sendVerifyLink" size="small"
                      >Resend</a-button
                    >
                  </div>
                </div>
          ...
          <div class="header-detail-plan" v-if="!isNotAdmin">
            ...
            <div
              ...
              :style="{
                color:
                  info.planInfo && info.planInfo.schoolInfo && info.planInfo.schoolInfo.planStatus === -1
                    ? '#ef4136'
                    : '#2F86FD'
              }"
              v-if="
                info.planInfo && info.planInfo.schoolInfo && [2, 3, -1].includes(info.planInfo.schoolInfo.planStatus)
              "
            >
              {{ expiredDay }}
            </div>
          ...
          <div class="header-detail-opt">
            <div class="header-detail-text">{{ consumedSize | sizeFormat }} of {{ totalSize | sizeFormat }}</div>
        ...
        <div class="info-self-detail">
          <div class="self-detail-name">
            <template v-if="!showSelfEdit">
              <label v-if="currentSchool.schoolUser.firstname || currentSchool.schoolUser.lastname"
                >{{ currentSchool.schoolUser.firstname }} {{ currentSchool.schoolUser.lastname }}</label
              >
            ...
            <template v-else>
              ...
              <a-input
                ...
                v-model="selfEditModel.lastname"
          ...
          <div class="self-detail-role">School roles: {{ roles }}</div>
        </div>
      </div>
      <!-- Basic info -->
      <div
        class="account-info-wrap"
        v-for="(item, index) in linkDatas.filter(_ => !_.hidden)"
        :key="'linkwrap_' + index"
      >
        ...
        <div class="account-info-link">
          ...
          >
            <guide-content :guideKey="link.guideKey">
...
export default {
    ...
    expiredDay() {
      ...
      if (this.info.planInfo) {
        ...
        if (this.info.planInfo.freeUsePlan || remain > 0) {
          date = moment
            .utc(this.info.planInfo.createTime)
            .local()
            .add(this.info.planInfo.freeDays, 'days')
            .format('YYYY-MM-DD HH:mm')
        } else {
          if (this.info.planInfo.planUser) {
            date = moment
              .utc(this.info.planInfo.planUser.planEndTime)
              .local()
              .format('YYYY-MM-DD HH:mm')
          }
        }
  ...
  methods: {
    ...mapMutations([HIDDEN_SIDEBAR, TOGGLE_USER_MODE]),
    ...
    hasRolePermission(permissionCode) {
      let hasPerm = false
      if (this.userMode === USER_MODE.SCHOOL && this.currentSchool.roleNames) {
        if (this.currentSchool.roleNames.includes('admin') || this.currentSchool.roleNames.includes('Admin'))
          return true
        let permissions = []
